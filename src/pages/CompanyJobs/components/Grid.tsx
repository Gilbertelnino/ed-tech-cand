...
import { getJobRequest, jobListRequest, jobDetailReset, jobAddFilter } from "../../../reducers/job/jobs.reducer";
...
const JobGrid = (props) => {
	...
	const handleCardClick = (id) => {
		if (id) {
			const companySlug = _.get(props, "match.params.slug", undefined);
			dispatch(getJobRequest({ id, companySlug, isPublic }));
			setOpen(true);
		}
	}
			...
			cards = (data || []).map((s) => {
				return (
					<Grid xs={12} sm={4} lg={3} key={s.id || ""} item>
						<Paper className="job-card-wrapper" onClick={() => handleCardClick(s.id)}>
